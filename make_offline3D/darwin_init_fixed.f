...
C Do not generate species in the continuous model (Le Gland, 13/01/2021)
C But some groups variables need initialization (Le Gland, 28/01/2021)
C#ifdef DARWIN_RANDOM_TRAITS
C      CALL DARWIN_GENERATE_RANDOM(myThid)
C#else
C      CALL DARWIN_GENERATE_ALLOMETRIC(myThid)
C#endif
       CALL DARWIN_GENERATE_CONSTANT(myThid)
...
C     WRITE(msgBuf,'(A)') ' DARWIN_INIT_FIXED: opening data.traits'
C     CALL PRINT_MESSAGE(msgBuf, standardMessageUnit,
C    &                   SQUEEZE_RIGHT , 1)

C     CALL MDSFINDUNIT( iUnit, myThid )
C     CALL OPEN_COPY_DATA_FILE(
C    I                   'data.traits', 'DARWIN_INIT_FIXED',
C    O                   iUnit,
C    I                   myThid )
