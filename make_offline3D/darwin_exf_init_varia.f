...
C       (usually done in darwin_fields_load, but may be needed for chl)
     ...
     &     starttime, nIter0, mythid )
        ...
        CALL EXCH_XY_RL (  surfPAR,  myThid  )

        CALL EXCH_XY_RL (  inputFe,  myThid  )

        CALL EXCH_XY_RL (  iceFrac,  myThid  )

        CALL EXCH_XY_RL (  windSpeed,  myThid  )

        CALL EXCH_XY_RL (  atmospCO2,  myThid  )
      ENDIF
