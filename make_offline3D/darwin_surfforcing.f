...
C Requires: SIZE.h
     ...
     &    pCO20, pCO21, atmospCO2
      ...
      Real*8 PAR0(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 PAR1(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 surfPAR(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)

      Real*8 iron0(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 iron1(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      ...
      Real*8 iceFrac(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      ...
      Real*8 pCO20(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 pCO21(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C  gO2  :: O2 tendency due to air-sea exchange
      Real*8  gDIC(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8  gALK(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8  gO2(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
...
C Solubility relation coefficients
      Real*8 SchmidtNoDIC(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 pCO2sat(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 Kwexch(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 Kwexch0(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 pisvel0(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
C local variables for carbon chem
      Real*8 surfdic
      Real*8 surfalk
      Real*8 surfphos
      Real*8 surfsi
      Real*8 surftemp(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 surfsalt(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 VirtualFlux(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 SchmidtNoO2(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 O2sat(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 aTT
      Real*8 aTK
      Real*8 aTS
      Real*8 aTS2
...
c pCO2 solver...
      DO j=jmin,jmax
       DO i=imin,imax
        IF ( maskC(i,j,k,bi,bj).NE.0.D0 ) THEN
c put bounds on tracers so pH solver doesn't blow up
...
c make sure Schmidt number is not negative (will happen if temp>39C)
         SchmidtNoDIC(i,j) = MAX(1.0D-2, SchmidtNoDIC(i,j))
...
C     print*, 'gDIC', gDIC, 'recip_drF', recip_drF, 'recip_hFacC',
C    & recip_hfacC, 'FluxCO2', FluxCO2, 'VirtualFlux', VirtualFlux
