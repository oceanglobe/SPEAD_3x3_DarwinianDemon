...
C  k                    :: vertical level
      ...
      Real*8 dTsub(Nr)
      Real*8 midTime, subTime
      Real*8 gPtr(1-OLx:sNx+OLx, 1-OLy:sNy+OLy, Nr, nDarwin)
      Real*8 PAR(1-OLx:sNx+OLx, 1-OLy:sNy+OLy, Nr, nlam)
      Real*8 diags(1-OLx:sNx+OLx,1-OLy:sNy+OLy, Nr, darwin_nDiag)
      Real*8 gDIC(1-OLx:sNx+OLx, 1-OLy:sNy+OLy)
      Real*8 gALK(1-OLx:sNx+OLx, 1-OLy:sNy+OLy)
      Real*8 gO2(1-OLx:sNx+OLx, 1-OLy:sNy+OLy)
      Real*8 surfFe(1-OLx:sNx+OLx, 1-OLy:sNy+OLy)
      Real*8 freeFe(1-OLx:sNx+OLx, 1-OLy:sNy+OLy, Nr)
      Real*8 scv,scav_poc
      Real*8 flx, POCl
      Real*8 ptr(nDarwin), gtr(nDarwin), PARl(nlam)
C     Real*8 chlout(nPhoto)
      Real*8 chlout(nphyp)
      Real*8 diagsl(darwin_nDiag)
C In the continuous model, the temperature dependance is not sufficient
C Derivatives also have to be computed. This will be done in darwin_plankton
C (Le Gland, 14/01/2021)
...
C     print*,'DARWIN_FORCING: entering:'
...
C Derivatives also have to be computed. This will be done in darwin_plankton
C (Le Gland, 14/01/2021)
C       CALL DARWIN_TEMPFUNC(Theta(i,j,k,bi,bj),
C    &         photoTempFunc, grazTempFunc, reminTempFunc,
C    &         mortTempFunc, mort2TempFunc,
C    &         uptakeTempFunc, myThid)
        DO iTr=1,nDarwin
         ptr(iTr) = Ptracer(i, j, k, bi, bj, iTr)
         gtr(iTr) = gPtr(i, j, k, iTr)
        ENDDO
        DO l=1,nlam
         PARl(l) = MAX(0D0, PAR(i, j, k, l))
        ENDDO
C     print*,'DARWIN_FORCING: before plankton loop:'
        CALL DARWIN_PLANKTON(
     I                 ptr,
     U                 gtr,
     O                 chlout, diagsl,
     I                 PARl,
C    I                 PARl, photoTempFunc,
C    I                 grazTempFunc,
C    I                 reminTempFunc,
C    I                 mortTempFunc, mort2TempFunc,
C    I                 uptakeTempFunc,
     I                 Theta(i,j,k,bi,bj),
     I                 subTime, myIter, myThid )
        DO l=1,darwin_nDiag
