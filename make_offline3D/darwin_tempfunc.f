...
C
C     6 1 used in Follows et al (2007) - without max of 1
C     6 2 used in Dutkiewicz et al (2009), Hickman et al, 
C                            Monteiro etal, Barton et al
C      gives all phyto a  specific temperature range
...
C In the continuous model, photoFun needs to be defined only for each
C group. However, derivatives are required (Le Gland, 13/01/2021)
C Derivatives will be computed numerically (Le Gland, 28/01/2021)
C     Real*8 photoFun(nplank)
C     Real*8 grazFun(nplank)
      Real*8 photoFun(nTrac)
      Real*8 grazFun(nTrac)
      Real*8 reminFun
      Real*8 mortFun
      Real*8 mort2Fun
      Real*8 uptakeFun
...
c !LOCAL VARIABLES: ====================================================
      Real*8 Tkel
      Real*8 TempAe, Tempref, TempCoeff, reminref
C     Real*8 phytoTempTol
      INTEGER j
...
c     TempAe    =   0.092D0
C More moderate heterotrophic response to temperature (Le Gland,
C 10/09/2021)
      TempAe       = 0.069D0  ! Value from Le Gland et al. 2021
      Tempref      = 20.0D0
      phytoTempTol = 5.0D0
cswd -- Eppley curve
      reminFun = MAX(1D-10, EXP(TempAe*(Temp-Tempref)))
C     DO j = 1, nplank
      DO j = 1, nTrac
        grazFun(j) = reminFun
...
C    &      (phytoTempOptimum(j)-Tempref))) *
     &      (phytoTempOptimum(j)-Tempref)) *
     &      EXP((Temp-phytoTempOptimum(j))/phytoTempTol(j)) *
C    &      ((phytoTempOptimum(j)+phytoTempTol(j)-Temp)/phytoTempTol(j))
