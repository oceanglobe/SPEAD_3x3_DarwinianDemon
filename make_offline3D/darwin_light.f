...
C !OUTPUT PARAMETERS: ==================================================
      Real*8 PAR(1-OLx:sNx+OLx, 1-OLy:sNy+OLy, Nr)
CEOP
...
C  k                    :: vertical level
      INTEGER i,j,k,jp
      Real*8 PARlat(1-OLy:sNy+OLy)
      Real*8 Chl(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 PARF(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 atten(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
...
C        convert W/m2 to uEin/s/m2
      ...
      IF (myiter .gt. -99 .AND. .NOT.DARWIN_useQsw) THEN
       DO j=jMin,jMax
        DO i=iMin,iMax
         PARF(i,j) = PARF(i,j) * (1.0D0 - iceFrac(i,j,bi,bj))
        ENDDO
       ENDDO
      ENDIF
...
C      DO jp=1,nPhoto
       DO jp=1,nphyp
        DO j=jMin,jMax
         DO i=iMin,iMax
C         Chl(i,j) = Chl(i,j) + MAX(0.0,
C    &                 Ptracer(i,j,k,bi,bj,ic+jp-1)*R_ChlC(jp))
          Chl(i,j) = Chl(i,j) + MAX(0.0,
     &    Ptracer(i,j,k,bi,bj,ic+iplank(jp)-1)*R_ChlC(iplank(jp)))
         ENDDO
        ENDDO
       ENDDO
