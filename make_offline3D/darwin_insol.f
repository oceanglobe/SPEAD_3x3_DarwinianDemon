...
C     PARWm2 :: daily-average PAR in W/m2
      Real*8 PARWm2(1-OLy:sNy+OLy)

C============== Local variables ============================================
      Real*8  solar, albedo, par
      Real*8  dayfrac, yday, delta
      Real*8  lat, sun1, dayhrs
      Real*8  cosz, frac, fluxi
      integer j
c
      solar = 1360.D0   ! W/m2 -- solar constant
      albedo = 0.6D0    ! W/W PAR range -- planetary albedo
      par = 0.4D0       !photosynthetically reactive frac
c
c find day (****NOTE for year starting in 1 Jan *****)
        dayfrac=mod(Time,360.D0*86400.D0)
     &                    /(360.D0*86400.D0)        !fraction of year
        yday = 2.0D0*3.1416D0*dayfrac               !convert to radians
        delta = (0.006918D0- (0.399912D0*cos(yday)) !declination (min.zenith angle)
     &          +(0.070257D0*sin(yday))                 !(paltridge+platt)
     &          -(0.006758D0*cos(2.0D0*yday))
     &          +(0.000907D0*sin(2.0D0*yday))
     &          -(0.002697D0*cos(3.0D0*yday))
     &          +(0.001480D0*sin(3.0D0*yday)) )
       do j=1-OLy,sNy+OLy
c latitude in radians
          lat=YC(1,j,1,bj)/180.D0*3.1416D0
          sun1 = -sin(delta)/cos(delta) * sin(lat)/cos(lat)
          if (sun1.le.-0.999D0) sun1=-0.999D0
          if (sun1.ge. 0.999D0) sun1= 0.999D0
          dayhrs = abs(acos(sun1))
          cosz = ( sin(delta)*sin(lat)+              !average zenith angle
     &            (cos(delta)*cos(lat)*sin(dayhrs)/dayhrs) )
          if (cosz.le.0.005D0) cosz=0.005D0
          if (returnsolz) THEN
            if (cosz.ge. 0.999D0) cosz= 0.999D0
            PARWm2(j) = acos(cosz)*180.D0/3.1416D0
          else
            frac = dayhrs/3.1416D0           !fraction of daylight in day
c daily average photosynthetically active solar radiation just below surface
            fluxi = solar*(1.0D0-albedo)*cosz*frac*par
c
c convert to PARWm2
            if (fluxi.gt.0.0D0) PARWm2(j)=fluxi
c very large for polar night
            if (fluxi.lt.0.00001D0) PARWm2(j)=0.00001D0
          endif
       enddo !j
c
      return
