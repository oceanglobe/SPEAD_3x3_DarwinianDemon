...
C components due to real freshwater flux
      Real*8 xoamp_fw, yoamp_fw, zoamp_fw
      Real*8 mass_fw, xcom_fw, ycom_fw, zcom_fw
      common /sbo_fw/ xoamp_fw, yoamp_fw, zoamp_fw,
     &             mass_fw, xcom_fw, ycom_fw, zcom_fw

C components due to seaice motion
      Real*8 xoamc_si, yoamc_si, zoamc_si
      Real*8 mass_si
...
C     FWLoad    :: real freshwater flux mass load (kg/m**2)
      integer bi, bj, i, j, k
      Real*8 lat, lon, darea, dvolume
      Real*8 ae, sbo_omega
      PARAMETER ( ae        = 6.3710D6    )
      PARAMETER ( sbo_omega = 7.292115D-5 )
      Real*8 UE(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 VN(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 UEice(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 VNice(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 Mload(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 GCload, FWload
C Tiled global sums
      Real*8 tile_FWload(nSx,nSy)
      Real*8 tile_sboarea(nSx,nSy)
      Real*8 tile_GCload(nSx,nSy)
      Real*8 tile_mass(nSx,nSy)
      Real*8 tile_xcom(nSx,nSy)
      Real*8 tile_ycom(nSx,nSy)
      Real*8 tile_zcom(nSx,nSy)
      Real*8 tile_xoamc(nSx,nSy)
      Real*8 tile_yoamc(nSx,nSy)
      Real*8 tile_zoamc(nSx,nSy)
      Real*8 tile_xoamp(nSx,nSy)
      Real*8 tile_yoamp(nSx,nSy)
      Real*8 tile_zoamp(nSx,nSy)
      Real*8 tile_xoamc_si(nSx,nSy)
      Real*8 tile_yoamc_si(nSx,nSy)
      Real*8 tile_zoamc_si(nSx,nSy)
      Real*8 tile_mass_si(nSx,nSy)
      Real*8 tile_mass_fw(nSx,nSy)
      Real*8 tile_xcom_fw(nSx,nSy)
      Real*8 tile_ycom_fw(nSx,nSy)
      Real*8 tile_zcom_fw(nSx,nSy)
      Real*8 tile_xoamp_fw(nSx,nSy)
      Real*8 tile_yoamp_fw(nSx,nSy)
      Real*8 tile_zoamp_fw(nSx,nSy)
      Real*8 tile_mass_gc (nSx,nSy)
C Pre-computed cos(lat), sin(lat), cos(lon), sin(lon)
      Real*8 COSlat(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 SINlat(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 COSlon(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 SINlon(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
CEOP
