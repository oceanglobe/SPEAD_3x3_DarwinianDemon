...
C Mutation rate of each species (Le Gland, 31/03/2021)
     ...
     &    biovol,
     &    group,
C Added "plank" and "iplank" (Le Gland, 15/03/2021)
     &    plank,
     &    iplank,
C    &    igroup,
     &    qcarbon,
...
C     print*,'DARWIN_READ_TRAITS: starting:'

C     DO jp = 1,nPlank
      DO jp = 1,nTrac
C       vmaxNH4(jp) = vmaxNH4(jp) * useNH4(jp)
C       vmaxNO2(jp) = vmaxNO2(jp) * useNO2(jp)
C       vmaxNO3(jp) = vmaxNO3(jp) * useNO3(jp)
...
C     print*,'DARWIN_READ_TRAITS: completed:'
