...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      katten_chl = 4.D-2
      parfrac = 0.4D0
      parconv = 1D0/0.2174D0
      tempnorm = 0.3D0
      TempAeArr = -4000.0D0
      TemprefArr = 293.15D0
      TempCoeffArr = 0.5882D0
      alpfe = 0.04D0
      scav = 0.4D0/pyear
      ligand_tot = 1.D-3
      ligand_stab = 2.D5
      freefemax = 0.4D-3
      scav_rat = 0.005D0 / pday
      scav_inter = 0.079D0
      scav_exp = 0.58D0
      scav_R_POPPOC = 1.1321D-4
      depthfesed = -1.0D0
      fesedflux = 1D-3 / pday
      fesedflux_pcm = 0.68D-3
      fesedflux_min = 0.5D-3 / pday
      R_CP_fesed = 106D0
      Knita = 1D0/(0.5D0*pday)
      Knitb = 1D0/(10D0*pday)
      PAR_oxi = 10D0
      Kdoc = 1D0/(100D0*pday)
      Kdop = 1D0/(100D0*pday)
      Kdon = 1D0/(100D0*pday)
      KdoFe = 1D0/(100D0*pday)
      KPOC = 1D0/(50D0*pday)
      KPOP = KPOC
      KPON = KPOC
      KPOFe = KPOC
      KPOSi = 1D0/(300D0*pday)
      wC_sink = 10D0/pday
      wP_sink = wC_sink
      wN_sink = wC_sink
      wFe_sink = wC_sink
      wSi_sink = wC_sink
      wPIC_sink = 15D0/pday
      Kdissc = 1D0/(300D0*pday)
      R_OP = 170D0
      R_OC = 170D0/120.0D0
      m3perkg = 1D0 / 1024.5D0
      surfSaltMinInit = 4.0D0
      surfSaltMaxInit = 50.0D0
      surfTempMinInit = -4.0D0
      surfTempMaxInit = 39.0D0
      surfDICMinInit = 10.0D0
      surfDICMaxInit = 4000.0D0
      surfALKMinInit = 10.0D0
      surfALKMaxInit = 4000.0D0
      surfPO4MinInit = 1.0D-10
      surfPO4MaxInit = 10.0D0
      surfSiMinInit = 1.0D-8
      surfSiMaxInit = 500.0D0
      surfSaltMin = 4.0D0
      surfSaltMax = 50.0D0
      surfTempMin = -4.0D0
      surfTempMax = 39.0D0
...
C     O2crit from Lipschultz et al 1990, DSR 37, 1513-1541:
      ...
      palat_min = 0D0
      inhib_graz = 1.0D0
      inhib_graz_exp = 0.0D0
      hillnumUptake = 1.0D0
      hillnumGraz = 1.0D0
      hollexp = 1.0D0
      phygrazmin = 120D-10
      pcoefO2 = 290.82D0/pday
      pmaxDIN = 20/pday
      ksatDIN = 0.01D0
      alpha_hydrol = 2.0D0
      yod = 0.2D0
      yoe = yod/467*4/(1-yod)*106D0
      ynd = 0.16D0
      yne = ynd/467*5/(1-ynd)*106D0
C      fnh4 = 0.1D0
C      ynh4 = 1/(16+467/6/fnh4)*106D0
C      yonh4 = fnh4/467*4/(1-fnh4)*106D0
C      fno2 = 0.1D0
C      yno2 = 1/(16+563/2/fno2)*106D0
C      yono2 = fno2/563*4/(1-fno2)*106D0
      depthdenit = 185.0D0
