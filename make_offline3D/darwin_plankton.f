...
C     Real*8 X02(nplank)
C     Real*8 X11(nplank)
      Real*8 X_m1(nplank,nTrait)
      Real*8 X_m2(nplank,nTrait)
      Real*8 X_m11(nplank,nCov)
...
C     Real*8 cv_unbounded
C Diagnostic variables derived from the trait distribution moments
C     Real*8 sd_bvol(nplank)
C     Real*8 sd_topt(nplank)
C     Real*8 cr_bvto(nplank)
      Real*8 sd_tr(nplank,nTrait)
      Real*8 cr_tr(nplank,nCov)
...
C     Real*8 dmbdt(nplank)
C     Real*8 dmtdt(nplank)
C     Real*8 dvbdt(nplank)
C     Real*8 dvtdt(nplank)
C     Real*8 dcbtdt(nplank)
      Real*8 dmndt(nplank,nTrait)
      Real*8 dvrdt(nplank,nTrait)
      Real*8 dcvdt(nplank,nCov)
...
C (Le Gland, 07/06/2021)
      dmp_time = 3600.0D0
C     dmp_time = 10800.0D0
C     dmp_time = 10800000000.0D0
...
C           vr_tr(j,jp) = MIN ( 700.0, MAX(0., vr_tr(j,jp) ) )
     ...
     &                    0.0D-3 * max_vr_tr(j,jp) + 2*numut_tr(j,jp)
     &                    - vr_tr(j,jp) ) ) / dmp_time
C            vr_tr(j,jp) = MIN ( max_vr_tr(j,jp) , MAX( 1.0D-3 *
C     &                    max_vr_tr(j,jp) + 2*numut_tr(j,jp),
C     &                    vr_tr(j,jp) ) )
C Supposedly makes variance stay at zero when set to zero (Boris 15/11/23)
            vr_tr(j,jp) = MAX(1.0D-5,MIN(max_vr_tr(j,jp),vr_tr(j,jp)))
